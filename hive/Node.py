...
class Node:
    ...
    def update_rep(self, i):
        ...
        self.R[i] = cached_comb + self.params['delta_r'] * sum(self.Ts[i] * self.Ts[idx])

    def update_QC(self, i):
        self.QC[i] = sigmoid_s(d(self.c[i], self.C[i]))

    def can_recommend(self, i):
        return self.Ts[i] >= self.params['trust_threshold'] or self.params['eta']**(d(self.current_c, self.crecv[i]) + self.Ts[i] * d(self.c[i], self.c_cached[i])) <= self.params['tau']
    ...
    def calc_T(self):
        if self.ever_got_recs:
            ...
            dists = sigmoid_s(d(self.current_c, self.c[idx]))
            indT = np.sum(dists * self.R[idx] *
    ...
    def transact(self, nodes, sp):
        self.calc_T()
        # if self.T >= self.params['trust_threshold']:
        fb = sp.serve()
